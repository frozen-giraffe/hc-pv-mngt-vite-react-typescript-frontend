...
import { firstSuperuser, firstSuperuserPassword } from "../tests/config.ts"
import { randomEmail, randomPassword } from "../tests/utils/random.ts"
import { logInUser, logOutUser, signUpNewUser } from "../tests/utils/user.ts"

const tabs = ["用户信息", "更改密码", "显示设置"]

// User Information

test("My profile tab is active by default", async ({ page }) => {
  await page.goto("/settings")
  await expect(page.getByRole("tab", { name: "用户信息" })).toHaveAttribute(
    "aria-selected",
    "true",
  )
})
